...
class IndexViewTests(TestCase):
    ...
    def test_expired_tasks_present(self):
        expired_tasks_count = Task.objects.filter(
            deadline__lte=timezone.now(),
            is_done=False).count()
        self.assertEqual(
            self.response.context["expired_tasks"].count(),
            expired_tasks_count)

    def test_percent_of_expired_tasks_present(self):
        tasks_count = Task.objects.filter(assigned_to=self.user).count()
        expired_tasks_count = Task.objects.filter(
            deadline__lte=timezone.now()).count()
        percent = round((expired_tasks_count / tasks_count) * 100)
        self.assertEqual(
            self.response.context["percent_of_expired_tasks"], percent)
    ...
    def test_percent_of_bugs_present(self):
        bugs_count = Task.objects.filter(
            task_type=self.task_type_bug,
            assigned_to=self.user).count()
        tasks_count = Task.objects.filter(assigned_to=self.user).count()
